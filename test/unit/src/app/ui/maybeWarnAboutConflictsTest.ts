...
describe('warning about conflicts', () => {
	...
	it('warns when requested and there are conflicts', () => {
		...
		maybeWarnAboutConflicts.default({ warnAboutConflicts, settingsPath, settingName, existingSetting, overridingSetting })
		...
		expect(warn.default).toHaveBeenCalledWith(expectedWarning)
	})
	...
	it('does not warn when not requested', () => {
		...
		expect(warn.default).not.toHaveBeenCalled()
	})
