...
The `make start` command serves a development version of the application on port 8080, as well as an integration test runner listening on port 9876. It opens tabs for these in a new Google Chrome window, along with tabs for the coverage report, the task management software used for the project, and finally the production app. Finally, `make start` opens the repository in Webstorm. 
