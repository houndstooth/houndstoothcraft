...
4) Add an entry to the `SetPatternSettingForTest` interface, further overloading the signature of methods which implement it such that they can set `NewSettings` onto the `NewSettingsName` key.
...
3) Add an entry to the `SetPatternSettingForTest` interface, further overloading the signature of methods which implement it such that they can set `Coolness`es onto any key in the `CoolnessTypedSettingsNames` union.
