...
./bin/shared/cmd_w_msgs.sh "./bin/test.sh" "testing" "Your tests are not passing locally. Please correct issues before pulling." "Tests passing."
...
./bin/shared/cmd_w_msgs.sh "./bin/test/unit_tests_and_cover.sh" "running unit tests and covering" "Unit tests failed, or coverage did not meet threshold." "Unit tests passed with latest code, and coverage report updated."
